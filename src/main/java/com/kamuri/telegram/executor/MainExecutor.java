...
public class MainExecutor implements CommandLineRunner {
    ...
    public void run(String ...args) throws InterruptedException {
        ...
        InlineKeyboardMarkupDTO markup = new InlineKeyboardMarkupDTO(button);
        ...
        telegramBot.registerMessageHandler("/start", s -> {
            System.out.println(s.getText());
            telegramBot.sendMessage(new SendMessageDTO(chatId, "Hello " + s.getFrom().getFirstName(), markup));
        });
        telegramBot.registerMessageHandler(s -> System.out.println(s));
        telegramBot.startPolling(1);
    }

    void handleCQ(CallbackQuery callbackQuery, MessageUpdate response) {
        ...
        try {
            ...
            if (callbackQuery.getData().equals("edit")) {
                var msg = new EditMessageDTO(
                    chatId, response.getResult().getMessageId(), "ababa"
                );
                telegramBot.editMessage(msg);
            }
        } catch (feign.FeignException.BadRequest badRequest) {
            System.out.println("Fail to answer callback: " + messageUtils.maskToken(badRequest.getMessage()));
        }
    }
}
