...
public class TelegramBot implements ITelegramBot {
    ...
    public Update getUpdate(@Nullable MessageDTO messageDTO) {
        return telegramClient
            .getUpdate(Optional.ofNullable(messageDTO)
                        .orElseGet(() -> new MessageDTO(lastUpdateID)));
    }
    ...
    @Override
    public void startPolling(Integer interval, @Nullable Consumer<Update> callback) throws InterruptedException {
        while (true) {
    ...
    public void process(Integer interval, @Nullable Consumer<Update> callback) {
        ...
        Optional.ofNullable(callback)
            .ifPresent(s -> s.accept(lastUpdate));
    }
