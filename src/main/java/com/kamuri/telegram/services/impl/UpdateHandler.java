...
public class UpdateHandler implements IUpdateHandler {
    ...
    /**
     * * Handles Callback updates, filtering the registered callbacks and calling the matching callback function.
    ...
    private void handleCallbackQuery(CallbackQuery cq) {
        callbackHandlers.stream()
            .sorted((x, y) -> x.getIdentifier().equals("*") && 
                !y.getIdentifier().equals("*") ? 1 : !x.getIdentifier().equals("*") &&
                y.getIdentifier().equals("*") ? -1 : 0)
    ...
    /**
     * Handles message updates, filtering the registered callbacks and calling the matching callback function.
    ...
    private void handleMessage(Message message) {
        messageHandlers.stream()
            .sorted((x, y) -> x.getIdentifier().equals("*") && 
                !y.getIdentifier().equals("*") ? 1 : !x.getIdentifier().equals("*") &&
                y.getIdentifier().equals("*") ? -1 : 0)
            .filter(s -> message.getText().startsWith(s.getIdentifier()) || s.getIdentifier().equals("*"))
            .findFirst()
    ...
    public void registerCallbackHandler(String expectedData, Consumer<CallbackQuery> callback) {
        if(callbackHandlers.stream()
            .anyMatch(s -> s.getIdentifier().equals(expectedData))) {
    ...
    public void registerMessageHandler(String pattern, Consumer<Message> callback) {
        if(messageHandlers.stream()
            .anyMatch(s -> s.getIdentifier().equals(pattern))) {
